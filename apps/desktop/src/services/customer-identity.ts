...
/**
 * O CNPJ/CPF sem mascara — a unica forma em que dois documentos podem ser comparados.
 *
 * O OMIE devolve o documento COM mascara ("06.020.284/0001-64") e o KyberRock grava so o
 * conteudo (o campo da tela normaliza antes de salvar). Comparar as duas formas letra a
 ...
 *
 * Tira a pontuacao, nao as letras: com o CNPJ alfanumerico (IN RFB 2.229/2024) as 12
 * primeiras posicoes podem ser letras, e um `replace(/\D/g, "")` colapsaria dois CNPJs
 * diferentes na mesma chave — o cadastro de um cliente adotaria o do outro no pull.
 * Maiusculo porque a comparacao e por igualdade exata dos dois lados.
 */
...
/**
 * O mesmo `documentKey`, em SQL, para comparar a coluna `document` de uma tabela com o
 * resultado de `documentKey(...)`.
 *
 ...
 */
export const DOCUMENT_KEY_SQL =
  "upper(replace(replace(replace(replace(COALESCE(document, ''), '.', ''), '-', ''), '/', ''), ' ', ''))";
