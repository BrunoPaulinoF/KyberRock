...
- **Entregar na fila NÃO é imprimir** (`services/windows-raw-printer.ts` + `RAW_SPOOL_SCRIPT` em
  `main.ts`): `WritePrinter` devolve sucesso assim que o spooler aceita os bytes, e o
  `webContents.print` do caminho gráfico faz o mesmo — os dois respondem "deu certo" com a
  impressora **pausada, offline, sem papel ou com a tampa aberta**. O cupom era gravado como
  `printed`, a tela não dizia nada, e do lado do operador a única evidência era papel nenhum:
  numa pedreira foram **seis números de cupom queimados na mesma operação** antes de alguém
  desconfiar que o problema não era o sistema. Não há como confirmar que o papel saiu (a térmica
  não responde), mas dá para perguntar ao Windows se a impressora está em condição de imprimir —
  e é essa a diferença entre "impresso" e "a MP-4200 TH está em pausa". O estado vem **no mesmo**
  `powershell.exe` que já envia os bytes (linha `KYBERROCK-PRINTER-STATE`), então não custa uma
  chamada nova. Duas regras: a lista de estados que reprovam é **fechada e conservadora** —
  `Printing`, `Busy`, `Processing` e `Warming Up` ficam de fora, senão impressão normal viraria
  erro na cara do operador —, e **diagnóstico que falha não reprova impressão que deu certo**
  (Windows sem o módulo de impressão, permissão negada → `unknown`, e o cupom segue). O caminho
  `windows` (HTML) ainda não tem essa verificação.
