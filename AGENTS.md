...
- **Conexao TCP da balanca** (`SCALE_CONNECTION_TUNING.timeoutMs`, `DEFAULT_CONNECT_TIMEOUT_MS`): a espera pelo aperto de mao **nao volta para 3000ms**. 3s e exatamente o tempo que o Windows leva para repetir o primeiro SYN, entao o app desistia no instante em que o sistema ia tentar de novo: um unico pacote perdido — rotina numa balanca por Wi-Fi ou num conversor ocupado respondendo ARP — virava "Timeout de conexao (3000ms)" na tela com o indicador ligado e a rede boa. Com 10s cabem as retransmissoes do sistema, e esperar mais nao custa nada porque a reconexao ja e automatica e sem limite de tentativas. A varredura da rede ("Procurar balanca") passa o seu proprio timeout curto (1200ms) e nao herda esse valor. O texto do erro tambem deixou de ser o do Node: `describeTcpConnectFailure` (`packages/scale-adapters/src/toledo/connection-diagnostics.ts`) separa porta errada (`ECONNREFUSED`, o IP esta certo), rede diferente (`EHOSTUNREACH`), sessao ocupada (`ECONNRESET` — o conversor aceita um cliente por vez) e ninguem respondendo, porque cada uma pede uma acao diferente de quem esta na pedreira. Na tela, o aviso **se apaga sozinho** quando a balanca volta a pesar: antes ele so era escrito e nunca limpo, e a faixa vermelha ficava la com a balanca ja conectada.
