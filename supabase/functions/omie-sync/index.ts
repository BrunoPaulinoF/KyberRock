...
async function checkOmieOrdersBilling(
  ...
  // Os que a nuvem ja deu por inexistentes nao chegam a virar chamada ao OMIE. Vem antes
  // de tudo porque poda a passada inteira, a varredura de notas inclusive: documento
  // excluido no OMIE nao tem nota para procurar.
  ...
  const salesWanted = orders.filter(
    (order) =>
      ...
      order.omieOrderId > 0 &&
      !knownMissing.has(missingDocumentKey("sales", order.omieOrderId))
  );
  const invoiceIndex =
