...
Eleger a principal não basta: o preço que ela já tinha está "atrás do cursor" do push incremental, e
a secundária pode ter preço que só existe nela. Por isso a troca arma **uma** passada em cada lado:
...
- **Secundária recém-criada** (`PRICE_MASTER_RESYNC_KEY`): o próximo pull vem **completo** (mesmo
  quando o ciclo pediu incremental — o delta diz o que a principal mudou, nunca o que ela não tem) e
  retira de cena o cadastro de preço que só existe naquela máquina.

O realinhamento da secundária **espera** enquanto a nuvem não tem preço nenhum publicado
(`hasPriceCadastroRows`): apagar ali deixaria a balança sem preço até a principal publicar, e preço
em branco custa mais caro para a operação que preço divergente.
...
divergente. As duas funções puras que resolvem isso vivem em `customer-freight-rules.ts`:
...
- `stripManualFreightValues` — no realinhamento, tira da regra a parte do cadastro e devolve `null`
  quando não sobra memória nenhuma (aí a linha inteira sai).
